...
- If you've enabled [`ENABLE_PER_KEY_SECRET`](/docs/settings#enable_per_key_secret), it also doesn't forge a *working* key at all: each key created with that setting on carries its own random secret, and only its SHA-256 hash is ever stored. `FERNET_SECRET` alone lets an attacker produce a token that decrypts successfully, but without the matching per-key secret it's still rejected. This is opt-in and only protects keys created after you enable it — it doesn't retroactively secure keys issued before that point.
